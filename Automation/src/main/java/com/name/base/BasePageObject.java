...
public abstract class BasePageObject<T> {
	...
	protected T verify_element(By locator, String expectedResult) {
		String ActualResult = verify(ExpectedConditions.presenceOfElementLocated(locator)).getText();
		Assert.assertEquals(ActualResult, expectedResult);
		return obj();
	}

	protected T verify_page_title() throws Error {
	...
	protected T browse_file(String filePath) {
		Robot robot = null;
		try {
			robot = new Robot();
	...
	protected T click_button(By locator) {
		click(locator);
		return obj();
	}

	protected T double_click_button(By locator) {
		Actions action = new Actions(driver);
		action.doubleClick(findElement(locator)).build().perform();
		return obj();
	}

	protected T right_click_button(By locator) {
		Actions action = new Actions(driver);
		//action.moveToElement(findElement(locator));
		action.contextClick(findElement(locator)).build().perform();
		return obj();
	}

	protected T submit(By locator) {
		WebElement submittable = findElement(locator);
		submittable.submit();
		return obj();
	}

	protected T select_by_index(By locator, int index) {
	...
	protected T select_by_name(By locator, String name) {
		Select selectable = findSelect(locator);
		selectable.selectByVisibleText(name);
		return obj();
	}

	protected T deselect_all(By locator) {
		Select selectable = findSelect(locator);
	...
	protected T verify_element_selected(By locator) {
	...
	protected T verify_element_by_text(By locator, String text) {
		verify(ExpectedConditions.textToBePresentInElementLocated(locator, text));
		return obj();
	}

	protected T verify_element_attribute_value(By locator, String attrName, String attrValue) {
		WebElement element = findElement(locator);
	...
	/*
	 * Command Use for Scroll Right In Testing Phase
	 */
	...
	/**
	 * Check the Select element is clickable and wait until loaded (i.e. size is >
	 * 1).
	 * 
	 * @param locator
	 *            (web element locator)
